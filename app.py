...
> 📌 **Konteks:** *Nilai Jual* = harga sampah mentah ke pabrik. *Biaya Proses* = biaya operasional industri. *Cost Burden* = biaya proses − nilai jual.
        """)
    ...
    with cr:
        ...
        fig_pie = px.pie(
            ...
            title='Komposisi Jenis Sampah per Kategori', hole=0.4
        )
    ...
    cols_show = ['No', 'Nama Sampah', 'Kategori', 'Emisi CO2e (kg)',
                 'cost_burden', 'high_cost_burden', 'kategori_urai']
    cols_avail = [c for c in cols_show if c in df_f.columns]
    st.dataframe(df_f[cols_avail].head(20), use_container_width=True)
...
# ════════════════════════════════════════════════════════
    ...
    <div class="main-header">
        ...
        <p>Distribusi dan profil statistik dataset Trashkara</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig_hist.update_layout(height=380, title_font_size=14,
                                xaxis_title='Emisi CO₂e (kg)', yaxis_title='Frekuensi')
        st.plotly_chart(fig_hist, use_container_width=True)
        st.caption("💡 Distribusi merata karena setiap jenis sampah memiliki emisi tetap (300 baris identik per jenis).")

    with col2:
        cb_cnt = df_f['cost_burden'].value_counts().sort_index().reset_index()
        cb_cnt.columns = ['Cost Burden', 'Jumlah']
        cb_cnt['Warna'] = cb_cnt['Cost Burden'].apply(
            lambda x: '#27ae60' if x <= 0 else ('#f1c40f' if x == 0 else '#e74c3c'))
        fig_cb = px.bar(
            cb_cnt, x='Cost Burden', y='Jumlah',
            title='Distribusi Cost Burden (Biaya Proses − Nilai Jual)',
            text='Jumlah', color='Cost Burden',
            color_continuous_scale='RdYlGn_r'
        )
        fig_cb.update_traces(texttemplate='%{text:,}', textposition='outside')
        fig_cb.update_layout(height=380, title_font_size=14,
                              xaxis_title='Cost Burden (Encoded)', yaxis_title='Frekuensi',
    ...
    with col3:
        ...
        fig_grouped.add_trace(go.Bar(
            ...
            marker_color='#27ae60', text=ekon['nilai_jual_encoded'].round(2), textposition='outside'
        ))
        fig_grouped.add_trace(go.Bar(
            name='Biaya Proses Industri', x=ekon['Kategori'], y=ekon['biaya_proses_encoded'],
            marker_color='#e74c3c', text=ekon['biaya_proses_encoded'].round(2), textposition='outside'
        ))
        ...
        st.caption("💡 Semua kategori: biaya proses (merah) > nilai jual mentah (hijau). Industri pengolahan sampah secara umum tidak mandiri finansial.")

    with col4:
        cb_cat = df_f.groupby('Kategori')['cost_burden'].mean().reset_index().sort_values('cost_burden')
        fig_hbar = px.bar(
            cb_cat, y='Kategori', x='cost_burden',
            color='Kategori', color_discrete_map=CAT_COLOR,
        ...
        st.caption("💡 B3 memiliki cost_burden tertinggi. Organik negatif — nilai jual > biaya proses (paling viable).")

    # Heatmap korelasi
    st.markdown('<div class="section-header">🔥 Heatmap Korelasi Numerik</div>', unsafe_allow_html=True)
    nc = ['Dapat Terurai', 'Daur Ulang', 'Nilai Jual', 'waktu_urai_bulan', 'kesulitan_encoded',
          'Emisi CO2e (kg)', 'nilai_jual_encoded', 'biaya_proses_encoded', 'cost_burden']
    nc_avail = [c for c in nc if c in df_f.columns]
    corr = df_f[nc_avail].corr().round(2)
    fig_corr = px.imshow(
        corr, text_auto=True, color_continuous_scale='RdYlGn',
        title='Heatmap Korelasi — Variabel Numerik & Encoded',
        aspect='auto', zmin=-1, zmax=1
    )
    fig_corr.update_layout(height=500, title_font_size=14)
    st.plotly_chart(fig_corr, use_container_width=True)
    st.caption("💡 'bp_encoded' dan 'cost_burden' berkorelasi sangat tinggi — cost_burden didominasi biaya proses. 'Daur Ulang' berkorelasi positif dengan 'nilai_jual_encoded'.")
...
# ════════════════════════════════════════════════════════
    ...
    st.markdown("""
    <div class="main-header">
        <h1>📋 Data Dictionary & Kesiapan Pemodelan</h1>
        <p>Dokumentasi kolom dan kesiapan dataset untuk Machine Learning</p>
    </div>
    """, unsafe_allow_html=True)

    base = os.path.dirname(os.path.abspath(__file__))
    dd_path = os.path.join(base, "data/data_dictionary.csv")
    try:
    ...
    mc = ['Dapat Terurai', 'Daur Ulang', 'Nilai Jual', 'waktu_urai_bulan', 'kesulitan_encoded',
          'Emisi CO2e (kg)', 'nilai_jual_encoded', 'biaya_proses_encoded', 'cost_burden', 'high_cost_burden',
    ...
    st.markdown('<div class="section-header">Statistik Deskriptif Fitur Numerik</div>', unsafe_allow_html=True)
    num_cols = [c for c in av if df[c].dtype in ['float64', 'int64']]
